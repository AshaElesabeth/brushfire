module.exports = function alreadyInUse (err){
  ...
  if (err.invalidAttributes.email) {
    ...
    return res.status(409).send({message: 'Email address is already taken by another user, please try again.'});
  }

  if (err.invalidAttributes.username) {
    //return res.send(409, 'Username is already taken by another user, please try again.');
    return res.status(409).send({message: 'Username is already taken by another user, please try again.'});
  }
};